    ...
    <EnableFeatureCard
      ...
      description={
        <>
          ...
          <br />
          Reply Manager only shows you the ones that do.
        </>
      }
      extraDescription={
      ...
      buttonText={enabled ? "Got it!" : "Enable Reply Manager"}
      onEnable={async () => {
        markOnboardingAsCompleted(REPLY_ZERO_ONBOARDING_COOKIE);

        if (enabled) {
          router.push(prefixPath(emailAccountId, "/reply-manager"));
          return;
        }
        ...
        if (result?.serverError) {
          toastError({
            title: "Error enabling Reply Manager",
            description: result.serverError,
          });
        } else {
          toastSuccess({
            title: "Reply Manager enabled",
            description: "We've enabled Reply Manager for you!",
          });
        }
        ...
        toast.promise(
          async () => {
            ...
            router.push(prefixPath(emailAccountId, "/reply-manager?enabled=true"));
          },
          {
            loading:
