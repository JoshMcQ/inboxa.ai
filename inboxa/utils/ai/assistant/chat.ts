...
export async function aiProcessAssistantChat({
  ...
  const system = `You are an assistant that helps create and update rules to manage a user's inbox. Our platform is called InboxaAI.
...
Reply Manager is a feature that labels emails that need a reply "To Reply". And labels emails that are awaiting a response "Awaiting". The user is also able to see these in a minimalist UI within InboxaAI which only shows which emails the user needs to reply to or is awaiting a response on.
...
<examples>
  ...
  <example>
    ...
    <output>
      InboxaAI doesn't support time-based actions yet. We only process emails as they arrive in your inbox.
    </output>
  </example>
